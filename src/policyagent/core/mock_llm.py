...
class MockLLMClient:
    ...
    def _analyzer_response(self, msg: str) -> LLMResponse:
        ...
        if "mutual exclusion" in msg.lower() or "cannot be billed" in msg.lower():
            rules.append(
                {
                    "id": "RULE-001",
                    "name": "Microsurgery Add-on Restriction",
                    "description": "CPT 69990 cannot be billed with non-microsurgery procedures",
                    "classification": "mutual_exclusion",
                    "source_text": "CPT 69990 cannot be billed with non-microsurgery procedures.",
                    "cpt_codes": ["69990"],
                    "icd10_codes": [],
                    "modifiers": [],
        ...
        if any(kw in msg.lower() for kw in ["overutilization", "maximum", "limit"]):
            rules.append(
                {
                    "id": "RULE-002",
                    "name": "Therapeutic Exercise Unit Limit",
                    "description": "CPT 97110 is limited to 4 units per day",
                    "classification": "overutilization",
                    "source_text": "CPT 97110 limited to max 4 units per day per patient.",
                    "cpt_codes": ["97110"],
                    "icd10_codes": [],
                    "modifiers": [],
                    "conditions": ["Maximum 4 units per day per patient"],
                }
            )
            ...
            rules.append(
                {
                    ...
                    "classification": "service_not_covered",
                    "source_text": "Cosmetic procedures (CPT 15780-15783) not covered.",
                    "cpt_codes": ["15780", "15781", "15782", "15783"],
                    "icd10_codes": [],
                    "modifiers": [],
            ...
            rules.append(
                {
                    ...
                    "classification": "mutual_exclusion",
                    "source_text": "Sample policy text",
                    "cpt_codes": ["99213"],
                    "icd10_codes": [],
                    "modifiers": [],
                    "conditions": [],
    ...
    def _sqlgen_response(self, msg: str) -> LLMResponse:
        if "69990" in msg or "mutual_exclusion" in msg.lower():
            sql = (
                "SELECT cl.* FROM claim_line cl JOIN claim c ON cl.claim_id = c.claim_id "
                "WHERE cl.cpt_code = '69990' AND NOT EXISTS ("
                "SELECT 1 FROM claim_line cl2 WHERE cl2.claim_id = cl.claim_id "
                "AND cl2.cpt_code IN ('61304', '61305', '61312', '61313'))"
            )
            exp = "Finds claims with 69990 lacking approved primary procedure"
        elif "97110" in msg or "overutilization" in msg.lower():
            sql = (
                "SELECT c.claim_id, cl.dos, SUM(cl.units) as total_units "
                "FROM claim_line cl JOIN claim c ON cl.claim_id = c.claim_id "
                "WHERE cl.cpt_code = '97110' GROUP BY c.claim_id, cl.dos, c.patient_id "
                "HAVING SUM(cl.units) > 4"
            )
            ...
            sql = (
                ...
                "WHERE cl.cpt_code IN ('15780', '15781', '15782', '15783')"
            )
            exp = "Finds claims with cosmetic procedure codes that are not covered"
        else:
            sql = "SELECT cl.* FROM claim_line cl WHERE cl.cpt_code = '99213'"
            exp = "Sample query"
        return LLMResponse(
    ...
    def _scorer_response(self, msg: str, tools: list[JSON] | None) -> LLMResponse:
        if tools and self._call_count % 2 == 1:
            ...
            return LLMResponse(
                content="I'll search for validation.",
                tool_calls=[
                    ToolCall(
                        id=f"call_{self._call_count}",
                        name="web_search",
                        arguments={"query": f"CMS {name} guidelines"},
                    )
                ],
        ...
        sources = [
            ...
            {
                "title": "AMA CPT Code Guidelines",
                "relevance": 0.85,
                "url": "https://www.ama-assn.org/practice-management/cpt",
                "snippet": "Official CPT...",
            },
        ]
        notes = ["Rule aligns with CMS guidelines", "Confirmed by industry standards"]
        import json

        response_json = json.dumps(
            {"confidence": conf, "sources": sources, "validation_notes": notes}
        )
        return LLMResponse(
            content=f"```json\n{response_json}\n```",
            tool_calls=[],
            finish_reason="stop",
            usage=TokenUsage(prompt_tokens=400, completion_tokens=250, total_tokens=650),
        )
