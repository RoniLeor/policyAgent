...
class HTMLTool(Tool):
    ...
    def get_parameters_schema(cls) -> dict[str, Any]:
        return {
            ...
            "properties": {
                "report_data": {
                    "type": "object",
                    "description": "Report data including rules and metadata.",
                },
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if not report_data:
            return ToolResult(
                tool_name=self.name, success=False, error="Missing required parameter: report_data"
            )
        if not output_path:
            return ToolResult(
                tool_name=self.name, success=False, error="Missing required parameter: output_path"
            )
        try:
            ...
            return ToolResult(
                tool_name=self.name,
                success=True,
                output={"path": str(output.absolute()), "size": len(html_content)},
            )
        except Exception as e:
            logger.exception("HTML generation failed")
            return ToolResult(
                tool_name=self.name, success=False, error=f"HTML generation failed: {e}"
            )
...
def create_default_template() -> None:
    ...
    from policyagent.templates.report_template import REPORT_TEMPLATE
